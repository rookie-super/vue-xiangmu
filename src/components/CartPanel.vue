<template>
    <li class="nav-cart" v-if="ZhuCeData.bool" @mouseenter="showCarPanel" @mouseleave="hideCarPanel">
        <a href="#" class="ball-rect">Shop Cart</a>
        <span class="cart-empty-num" v-show="CartPanelData.length!=0">
                            <i>{{CartPanelData.length}}</i>
                        </span>
        <div class="nav-cart-wrapper" v-show="bool">
            <div class="nav-cart-list">
                <div class="empty" v-show="CartPanelData.length==0">
                    <h3>没有商品</h3>
                    <p>买买买！</p>
                    <router-link to='/cart' class="nav-cart-btn"><span>去购物车</span></router-link>
                </div>
                <div class="full" v-show="CartPanelData.length!=0">
                    <div class="nav-cart-items">
                        <ul>
                            <li class="clear" v-for="(item ,index) in CartPanelData">
                                            ...
                                            <div class="cart-cell">
                                                ...
                                                <h6>
                                                    <span class="price-icon">￥</span><span
                                                        class="price-num">{{item.money}}</span><span
                                                        class="item-num">x {{item.num}}</span>
                                                </h6>
                                            </div>
                                        </div>
                                        <div class="del-btn" @click="deleteData(item.id)">删除</div>
                                    </div>
                                </div>
                            </li>
                    ...
                    <div class="nav-cart-total">
                        <p>数量: <strong class="ng-binding">{{CartPanelData.length}}</strong></p>
                        <h5>总计：<span class="price-icon" style="font-size:18px;">￥ </span><span
                                class="price-num ng-binding" ng-bind="cartMenu.totalPrice">{{zongJia}}</span>
                        </h5>
                        <h6>
                            <router-link to='/cart' class="nav-cart-btn"><span>去购物车</span></router-link>
                        </h6>
...
export default {
    ...
    computed:{
        ...mapState(['CartPanelData','ZhuCeData']),
        zongJia(){
            let arr=this.CartPanelData;
            let sum=0;
            for(let i=0;i<arr.length;i++){
                sum+=arr[i].money*arr[i].num
            }
            return sum
        }
    },
...
<style >
    ...
    .nav-logo a {
        ...
        width: 130px;
        height: 50px;
        background: url(../assets/img/core-img/logo2.png) no-repeat;
