        ...
        <div class="content page-order-payment">
            <div class="gray-box clear">
                ...
                <div class="box-inner order-info">
                    <h3>订单提交成功</h3>
                    <p class="payment-detail">请您<span>24 小时</span> 内支付。</p>
                </div>
                <div class="box-inner payment-checkout-panel clear"><span
                        class="jianguo-blue-main-btn big-main-btn js-payment-order" @click="payOrder"><a>支付</a></span><span
                        class="prices"> 支付金额： <em><span>￥ </span>{{order.sum}}</em></span></div>
            </div>
            <div class="confirm-detail">
                <div class="info-title">订单号</div>
                <p class="info-detail">{{order.orderNum}}</p>
            </div>
            <div class="confirm-detail" v-if="order.diZhi">
                <div class="info-title">收货地址</div>
                <p class="info-detail">姓名：{{order.diZhi.name}}</p>
                <p class="info-detail">电话：{{order.diZhi.phone}}</p>
                <p class="info-detail">地址：{{order.diZhi.city}}{{order.diZhi.site}}
                </p>
            </div>
            <div class="confirm-table-title clear"><span class="name fn-left">商品名称</span><span
                    class="subtotal fn-right">小计</span><span class="num fn-right">数量</span><span
                    class="price fn-right">单价</span></div>
            <div class="confirm-goods-table">
                <div class="cart-items clear"  v-for="item in order.listData">
                    <div class="name fn-left hide-row">
                        <div class="name-cell">
                            <a href="javascript:;" title="" target="_blank">{{item.name}}</a><br>
                        </div>
                    </div>
                    <div class="subtotal fn-right">
                        <div class="subtotal-cell">￥ {{item.num*item.money}}.00</div>
                    </div>
                    <div class="goods-num fn-right">{{item.num}}</div>
                    <div class="price fn-right">￥ {{item.money}}.00</div>
                </div>
            </div>
            <div class="order-discount-line">
                <p> 总价： <span>¥ {{order.sum}}</span></p>
            </div>
        </div>
    </div>
</template>
...
<script>
    export default {
        ...
        methods:{
            payOrder(){
                this.$store.commit("payOrder",this.order.orderNum);
                alert("假装支付成功")
            }
        }
    }
