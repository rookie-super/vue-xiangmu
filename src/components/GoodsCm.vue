        ...
        <div class="store-content item">
            <div class="item-box">
                <div class="gallery-wrapper">
                    <div class="gallery" v-if="items.imgs">
                        <div class="thumbnail">
                            <ul>
                                <li v-for="(item , index) in items.imgs" @click="chooseId(index)" :class="{on:img_index===index}">
                                    <img :src="item" >
                                </li>
                        ...
                        <div class="thumb">
                            <ul>
                                <li  class="on" v-if="items.imgs">
                                    <img :src="items.imgs[img_index]" width="400">
                                </li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="banner">
                    <div class="sku-custom-title">
                        <div class="params-price">
                            <span><em style="font-size:25px;">￥</em><i>{{items.moneyNow}}</i></span>
                        </div>
                        ...
                        <div class="sku-dynamic-params clear">
                            ...
                            <div class="params-detail clear">
                                <div class="item-num js-select-quantity">
                                    <span class="down" @click="jian">-</span>
                    ...
                    <div class="sku-status">
                        <div class="cart-operation-wrapper clearfix">
                            <span class="blue-title-btn js-add-cart" @click="addCartPanel">加入购物车</span>
                            <span class="gray-title-btn" @click="addCheckOutData">立即购买</span>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        name: "Goods",
        computed:{...mapState(["ZhuCeData"])},
        props:["items","idGood","idGoods"],
        data:function () {
            return{
                img_index:this.idGoods-1,
                count:1
            }
        },
        methods:{
            chooseId(index){
                ...
                this.idGoods=index+1;
                this.$store.dispatch("getGoodsAsync",{ids:this.idGood,id:this.idGoods})
            },
            jian(){
                this.count--;
                if(this.count<0){
            ...
            add(){
                this.count++;
                if(this.count>this.items.num){
                    this.count=this.items.num
                }
            },
            addCartPanel(){
                if(this.ZhuCeData.bool){
                    let json={
                        id:this.items.id,
                        name:this.items.name,
                        num:this.count,
                        img:this.items.img,
                        money:this.items.moneyNow
                    };
                    this.$store.commit("getCartPanelData",json)
                }else {
                    alert("请先登录账号")
                }
            },
            addCheckOutData(){
                if(this.ZhuCeData.bool){
                    ...
                    this.$store.commit("getCheckOutData",{bool:false,data:[json]});
